...
def convert_decimal(content: dict[str, Any]) -> dict[str, Any]:
    ...
    if 'embeddings' in content:
        content['embeddings'] = [
            [float(x) if isinstance(x, Decimal) else x for x in embedding] if embedding else embedding
            for embedding in content['embeddings']
        ]
    if 'average_embedding' in content:
        if content['average_embedding']:
            content['average_embedding'] = [float(x) if isinstance(x, Decimal) else x for x in content['average_embedding']]
    return content
